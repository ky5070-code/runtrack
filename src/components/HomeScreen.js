...
function CreateSetModal({ onClose, onCreate }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#0d0d0d", borderRadius: "22px 22px 0 0", border: "1px solid #1a1a1a", padding: "20px 20px", paddingBottom: `calc(24px + ${safeBottom})`, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 22 }}>
          {[[true, "🌍 공개", "누구나 검색하고 입장 가능"], [false, "🔒 비공개", "링크로만 입장 가능"]].map(([val, label, desc]) => (
            <button key={String(val)} onClick={() => setIsPublic(val)} style={{ flex: 1, padding: "12px 10px", borderRadius: 14, border: isPublic === val ? "1.5px solid #00ff88" : "1px solid #1a1a1a", background: isPublic === val ? "#0d1f14" : "#080808", textAlign: "left" }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: isPublic === val ? "#00ff88" : "#555", marginBottom: 4 }}>{label}</div>
              <div style={{ fontSize: 10, color: "#2a2a2a", lineHeight: 1.4 }}>{desc}</div>
            </button>
          ))}
        </div>
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#0d0d0d", borderRadius: "22px 22px 0 0", border: "1px solid #1a1a1a", padding: "20px 20px", paddingBottom: `calc(24px + ${safeBottom})`, maxHeight: "85vh", overflowY: "auto" }}>
        ...
        {filtered.map(s => (
          <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px", background: "#080808", border: "1px solid #111", borderRadius: 14, marginBottom: 8 }}>
            <div style={{ width: 48, height: 48, borderRadius: 24, background: "#0d1f14", border: "1px solid #1a3d28", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>{s.emoji}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700 }}>{s.name}</div>
              {s.description && <div style={{ fontSize: 11, color: "#444", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.description}</div>}
              <div style={{ fontSize: 10, color: "#2a2a2a", marginTop: 3 }}>멤버 {s.memberIds?.length || 0}명</div>
            </div>
            <button onClick={() => handleJoin(s)} disabled={joining === s.id} style={{ padding: "8px 16px", background: "#00ff88", border: "none", borderRadius: 10, color: "#000", fontFamily: "inherit", fontSize: 12, fontWeight: 800, minHeight: 38, flexShrink: 0 }}>
              {joining === s.id ? "..." : "입장"}
            </button>
          </div>
        ))}
    ...
    <div style={{ minHeight: "100vh", background: "#060606", color: "#e0e0e0", fontFamily: "'Nanum Gothic', sans-serif", maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={{ padding: "14px 20px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: "#00ff88", letterSpacing: -0.5 }}>HOME</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{currentUser?.name}</div>
            <button onClick={onLogout} style={{ background: "none", border: "none", color: "#333", fontFamily: "inherit", fontSize: 10, cursor: "pointer", padding: 0 }}>로그아웃</button>
          </div>
          <div style={{ width: 42, height: 42, borderRadius: 21, background: "#111", border: "1.5px solid #222", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{currentUser?.avatar || "🏃"}</div>
        </div>
      </div>
      ...
      <div style={{ padding: "20px 20px", paddingBottom: `calc(24px + ${safeBottom})` }}>
        ...
        {mySets.map(set => (
          <div key={set.id} onClick={() => onEnterSet(set)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px", background: "#0a0a0a", border: `1px solid ${set.adminId === currentUser?.uid ? "#1a3d28" : "#161616"}`, borderRadius: 18, marginBottom: 10, cursor: "pointer" }}>
            <div style={{ width: 56, height: 56, borderRadius: 18, background: set.adminId === currentUser?.uid ? "#0d1f14" : "#0d0d0d", border: `1.5px solid ${set.adminId === currentUser?.uid ? "#1a3d28" : "#1a1a1a"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, flexShrink: 0 }}>{set.emoji}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 800, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{set.name}</div>
                {set.adminId === currentUser?.uid && <span style={{ background: "#0d1f14", border: "1px solid #1a3d28", borderRadius: 4, padding: "1px 6px", fontSize: 9, color: "#00cc66", flexShrink: 0 }}>관리자</span>}
                {!set.isPublic && <span style={{ fontSize: 11, color: "#2a2a2a", flexShrink: 0 }}>🔒</span>}
              </div>
