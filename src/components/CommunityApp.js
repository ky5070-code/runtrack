...
function Toast({ message, type }) {
  const bg = type === "success" ? "#22D3EE" : type === "error" ? "#ff4444" : "#ffaa00";
...
function ConfirmModal({ message, onConfirm, onCancel, confirmLabel, confirmColor }) {
  const btnColor = confirmColor || "#22D3EE";
  return (
      ...
      <div style={{ padding: "14px 14px 12px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <div style={{ position: "relative" }}>
            ...
            {isMyPost && <div style={{ position: "absolute", inset: -2, borderRadius: 23, border: "2px solid var(--accent)", pointerEvents: "none" }} />}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 2, alignItems: "center" }}>
              ...
              {post.source === "ai" && <span style={{ background: "var(--accent-bg)", border: "1px solid var(--accent-bd)", borderRadius: 4, padding: "1px 6px", fontSize: 11, color: "var(--accent2)" }}>AI</span>}
              {post.appName && <span style={{ fontSize: 12, color: "var(--text4)" }}>{post.appName}</span>}
            </div>
          </div>
          {canDelete && (
            <button onClick={() => setShowMenu(true)} style={{ background: "none", border: "none", color: "var(--text5)", fontSize: 20, minWidth: 36, minHeight: 36, display: "flex", alignItems: "center", justifyContent: "center" }}>⋯</button>
          )}
        </div>
          ...
          <div onClick={() => setShowMenu(false)} style={{ position: "fixed", inset: 0, background: "var(--overlay)", zIndex: 400, display: "flex", alignItems: "flex-end" }}>
            <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "var(--bg4)", borderRadius: "20px 20px 0 0", padding: "20px 16px", paddingBottom: "calc(20px + env(safe-area-inset-bottom, 0px))" }}>
              ...
              <button onClick={() => { setShowMenu(false); setShowDeleteConfirm(true); }}
                style={{ width: "100%", padding: "15px", borderRadius: 12, border: "none", background: "#1a0808", color: "#ff4444", fontFamily: "inherit", fontSize: 16, fontWeight: 700, textAlign: "left" }}>
                🗑️ 삭제
              </button>
              <button onClick={() => setShowMenu(false)}
                style={{ width: "100%", padding: "15px", borderRadius: 12, border: "none", background: "transparent", color: "var(--text4)", fontFamily: "inherit", fontSize: 16, marginTop: 4 }}>
                취소
              </button>
            </div>
          </div>
        ...
        <div style={{ background: isMyPost ? "var(--accent-bg)" : "var(--card)", border: isMyPost ? "1px solid #1a3020" : "1px solid #141414", borderRadius: 14, padding: "12px 14px", marginBottom: 12, display: "flex" }}>
          {[[Number(post.dist).toFixed(2) + "km", "거리", true], [fmtTime(post.duration), "시간", false], [post.pace || "--", "페이스", false], [post.calories || "--", "칼로리", false]].map(([v, l, accent], i) => (
            <div key={l} style={{ flex: 1, borderLeft: i > 0 ? "1px solid #141414" : "none", paddingLeft: i > 0 ? 10 : 0 }}>
              ...
              <div style={{ fontSize: i === 0 ? 17 : 13, fontWeight: 800, color: accent ? "#22D3EE" : "#d0d0d0" }}>{v}</div>
            </div>
          ))}
        </div>

        {post.caption && <div style={{ fontSize: 15, color: "var(--text2)", marginBottom: 12, lineHeight: 1.6 }}>{post.caption}</div>}
        ...
        {post.aiFeedback && (
          <div style={{ marginBottom: 12 }}>
            <button onClick={() => setShowFeedback(s => !s)} style={{ width: "100%", background: "var(--accent-bg)", border: "1px solid var(--accent-bd)", borderRadius: showFeedback ? "12px 12px 0 0" : 12, padding: "10px 14px", display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontFamily: "inherit" }}>
              ...
              <span style={{ fontSize: 12, color: "var(--accent2)", fontWeight: 700, letterSpacing: 1 }}>AI 코치 피드백</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 0 }}>
              ...
              {EMOJIS.map(e => {
                ...
                return (
                  <button key={e} onClick={() => onReact(post.id, e)} style={{
                    ...
                    background: active ? "#0d1e24" : "#0d0d0d",
          ...
          {/* 댓글 버튼 - 항상 오른쪽 고정 */}
          <button onClick={() => setShowComments(s => !s)} style={{ background: "none", border: "none", color: "var(--text3)", fontSize: 16, minWidth: 40, minHeight: 40, flexShrink: 0, display: "flex", alignItems: "center", gap: 3 }}>
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12, marginTop: 10 }}>
            {(post.comments || []).map(c => (
              <div key={c.id} style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1, background: "var(--modal-bg)", borderRadius: 10, padding: "8px 12px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text3)" }}>{c.userName}</div>
                    {c.userId === currentUser?.uid && (
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => { setEditingCommentId(c.id); setEditingCommentText(c.text); }}
                          style={{ background: "none", border: "none", color: "var(--text4)", fontSize: 12, padding: "0 2px", cursor: "pointer" }}>수정</button>
                        <button onClick={() => onDeleteComment && onDeleteComment(post.id, c.id)}
                          style={{ background: "none", border: "none", color: "var(--text5)", fontSize: 12, padding: "0 2px", cursor: "pointer" }}>✕</button>
                      </div>
                    )}
                  </div>
                  {editingCommentId === c.id ? (
                    <div style={{ marginTop: 6 }}>
                      <input value={editingCommentText} onChange={e => setEditingCommentText(e.target.value)}
                        autoFocus
                        style={{ width: "100%", background: "var(--bg4)", border: "1px solid var(--accent)", borderRadius: 8, padding: "7px 10px", color: "var(--text1)", fontFamily: "inherit", fontSize: 14, outline: "none", boxSizing: "border-box", marginBottom: 6 }} />
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => { onEditComment && onEditComment(post.id, c.id, editingCommentText); setEditingCommentId(null); }}
                          style={{ flex: 1, background: "var(--accent)", border: "none", borderRadius: 8, padding: "7px", color: "#000", fontFamily: "inherit", fontSize: 13, fontWeight: 800 }}>저장</button>
                    ...
                    <div style={{ fontSize: 15, color: "var(--text2)", lineHeight: 1.5 }}>{c.text}</div>
                  )}
                </div>
              </div>
            ))}
            ...
            {showCommentInput ? (
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 8 }}>
                ...
                <button onClick={() => { submitComment(); setShowCommentInput(false); }}
                  style={{ width: 32, height: 32, borderRadius: 16, background: commentText.trim() ? "#22D3EE" : "var(--border)", border: "none", color: commentText.trim() ? "#000" : "#444", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/>
                  </svg>
                </button>
                <button onClick={() => setShowCommentInput(false)}
                  style={{ background: "none", border: "none", color: "var(--text5)", fontSize: 18, padding: "0 2px" }}>✕</button>
              </div>
            ) : (
              <button onClick={() => setShowCommentInput(true)}
                style={{ width: "100%", marginTop: 8, padding: "8px 12px", borderRadius: 20, border: "1px solid #1e1e1e", background: "var(--bg2)", color: "var(--text5)", fontFamily: "inherit", fontSize: 14, textAlign: "left", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {/* 댓글 입력 모달 */}
        {showCommentInput && (
          <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 9999, background: "var(--bg4)", borderTop: "1px solid #222", padding: "10px 14px", paddingBottom: "calc(10px + env(safe-area-inset-bottom, 0px))" }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center", maxWidth: 480, margin: "0 auto" }}>
              ...
              <button onClick={() => { submitComment(); setShowCommentInput(false); }}
                style={{ width: 34, height: 34, borderRadius: 17, background: commentText.trim() ? "#22D3EE" : "var(--border)", border: "none", color: commentText.trim() ? "#000" : "#444", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
...
function NoticeBoard({ notices, isAdmin, onAdd, onDelete }) {
  ...
  return (
    <div style={{ background: "var(--bg)", border: "1px solid var(--accent-bd)", borderRadius: 16, padding: "14px 16px", marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: notices.length > 0 || showInput ? 12 : 0 }}>
        ...
        {isAdmin && (
          <button onClick={() => setShowInput(s => !s)}
            style={{ background: showInput ? "#164e63" : "none", border: "1px solid var(--accent-bd)", borderRadius: 8, padding: "4px 10px", color: "var(--accent2)", fontFamily: "inherit", fontSize: 12, fontWeight: 700 }}>
            {showInput ? "취소" : "+ 공지 작성"}
          </button>
        )}
      </div>
      ...
      {showInput && (
        <div style={{ marginBottom: 12 }}>
          ...
          <button onClick={handleAdd} disabled={!text.trim() || loading}
            style={{ width: "100%", padding: "11px", borderRadius: 10, border: "none", background: text.trim() ? "#22D3EE" : "#111", color: text.trim() ? "#000" : "#333", fontFamily: "inherit", fontSize: 14, fontWeight: 800 }}>
            {loading ? "등록 중..." : "공지 등록"}
          </button>
        </div>
      )}
...
function UploadModal({ onClose, onPost, currentUser, isPro }) {
  ...
  const ef = (key, label, unit, kbd = "decimal") => (
    <div style={{ flex: 1 }}>
      <div style={{ fontSize: 12, color: "var(--text4)", marginBottom: 5 }}>{label}</div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 2 }}>
        <input value={edited[key] ?? ""} onChange={e => setEdited(p => ({ ...p, [key]: e.target.value }))} inputMode={kbd}
          style={{ width: "100%", background: "transparent", border: "none", borderBottom: "1px solid #222", color: "var(--accent)", fontFamily: "inherit", fontSize: 19, fontWeight: 800, outline: "none", padding: "4px 0" }} />
        {unit && <span style={{ fontSize: 12, color: "var(--text5)" }}>{unit}</span>}
      </div>
    </div>
  );
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "var(--overlay)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "var(--modal-bg)", borderRadius: "22px 22px 0 0", border: "1px solid var(--border)", padding: "20px 20px", paddingBottom: `calc(24px + ${safeBottom})`, maxHeight: "92vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          ...
          <button onClick={onClose} style={{ background: "var(--bg4)", border: "none", borderRadius: "50%", width: 34, height: 34, color: "var(--text3)", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        </div>
        ...
        {step === "pick" && <>
          ...
          <div onClick={() => inputRef.current?.click()} style={{ border: "2px dashed #1e1e1e", borderRadius: 18, padding: "44px 20px", textAlign: "center", background: "var(--bg)" }}>
          ...
          <div style={{ textAlign: "center", padding: "28px 0" }}>
            {previewUrl && <div style={{ position: "relative", width: 120, height: 120, borderRadius: 18, overflow: "hidden", margin: "0 auto 20px" }}>
              ...
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ width: 44, height: 44, border: "2px solid var(--accent)", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
              </div>
            </div>}
        ...
        {step === "confirm" && <>
          ...
          {isPro && feedback && (
            <div style={{ background: "var(--accent-bg)", border: "1px solid var(--accent-bd)", borderRadius: 14, padding: "14px 16px", marginBottom: 16 }}>
              <div style={{ fontSize: 12, color: "var(--accent)", letterSpacing: 2, marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>
                <span>✨</span> AI 코치 피드백
              </div>
              {feedback.split("\n").filter(l => l.trim()).map((line, i) => (
                <div key={i} style={{ fontSize: 15, color: "var(--text2)", lineHeight: 1.7, marginBottom: i < 2 ? 4 : 0 }}>{line}</div>
              ))}
            </div>
          )}
          {isPro && !feedback && result && (
            <div style={{ background: "var(--bg)", border: "1px solid #161616", borderRadius: 14, padding: "12px 16px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ fontSize: 14, color: "var(--text5)" }}>AI 코치가 피드백 작성 중...</div>
            </div>
          )}
          {!isPro && result && (
            <div style={{ background: "var(--bg2)", border: "1px solid var(--border2)", borderRadius: 14, padding: "14px 16px", marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ fontSize: 22 }}>🔒</div>
              <div>
                <div style={{ fontSize: 13, color: "var(--text3)", fontWeight: 700, marginBottom: 3 }}>AI 코치 피드백</div>
                <div style={{ fontSize: 12, color: "var(--text5)" }}>PRO 회원 전용 기능이에요</div>
              </div>
            </div>
          )}
          <div style={{ fontSize: 12, color: "var(--text5)", letterSpacing: 2, marginBottom: 14 }}>데이터 확인 · 수정 가능</div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => { setStep("pick"); setPreviewUrl(null); setFile(null); setResult(null); }} style={{ flex: 1, padding: "14px", background: "transparent", border: "1px solid #1e1e1e", borderRadius: 14, color: "var(--text3)", fontFamily: "inherit", fontSize: 16, minHeight: 52 }}>다시</button>
            <button onClick={() => setStep("caption")} style={{ flex: 2, padding: "14px", background: "var(--accent)", border: "none", borderRadius: 14, color: "#000", fontFamily: "inherit", fontSize: 17, fontWeight: 800, minHeight: 52 }}>다음 →</button>
          </div>
        </>}
        ...
        {step === "caption" && <>
          ...
          <button onClick={handlePost} disabled={posting} style={{ width: "100%", marginTop: 14, padding: "17px", background: posting ? "#0d3320" : "#22D3EE", border: "none", borderRadius: 14, color: posting ? "#22D3EE" : "#000", fontFamily: "inherit", fontSize: 17, fontWeight: 800, minHeight: 56 }}>
...
function LeaderboardTab({ posts, currentUser, isPro }) {
  ...
  posts.forEach(p => {
    ...
    if (p.userId === currentUser?.uid) userMap[p.userId].user = { ...p.author, ...currentUser };
  ...
  return (
    <div>
        ...
        {[["week", "이번 주"], ["month", "이번 달"], ["all", "전체"]].map(([v, l]) => {
          ...
          return (
            <button key={v} onClick={() => !locked && setPeriod(v)}
              style={{ flex: 1, padding: "11px", borderRadius: 12, border: locked ? "1px solid #1a1a1a" : "none", background: period === v ? "#22D3EE" : "#0d0d0d", color: period === v ? "#000" : locked ? "#333" : "#444", fontFamily: "inherit", fontSize: 13, fontWeight: 700, minHeight: 46 }}>
              {locked ? "🔒 " : ""}{l}
            </button>
          );
        })}
          ...
          {[scores[1], scores[0], scores[2]].map((s, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: rank === 0 ? "#22D3EE" : "#888", textAlign: "center" }}>{s.user.name}</div>
      ...
      {scores.map((s, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px", background: s.user.uid === currentUser?.uid ? "var(--accent-bg)" : "var(--bg3)", border: `1px solid ${s.user.uid === currentUser?.uid ? "#155e75" : "#161616"}`, borderRadius: 14, marginBottom: 8, minHeight: 66 }}>
          <div style={{ fontSize: i < 3 ? 22 : 14, width: 28, textAlign: "center", color: i < 3 ? "inherit" : "var(--text4)", fontWeight: 700 }}>{i < 3 ? medals[i] : i + 1}</div>
          <Avatar user={s.user} size={44} />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 16, fontWeight: 700 }}>{s.user.name}{s.user.uid === currentUser?.uid && <span style={{ color: "var(--accent)", fontSize: 12, marginLeft: 6 }}>ME</span>}</div>
            <div style={{ fontSize: 13, color: "var(--text4)", marginTop: 2 }}>{s.runs}회 러닝 · 평균 {s.avgPace}/km</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: i === 0 ? "var(--accent)" : "var(--text2)" }}>{s.dist.toFixed(1)}</div>
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "var(--modal-bg)", borderRadius: "22px 22px 0 0", border: "1px solid var(--border)", padding: "20px 20px", paddingBottom: `calc(24px + ${safeBottom})`, maxHeight: "80vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            {notifications.some(n => !n.read) && (
              <button onClick={onMarkAllRead} style={{ background: "none", border: "none", color: "var(--accent)", fontFamily: "inherit", fontSize: 14, cursor: "pointer" }}>모두 읽음</button>
            )}
        ...
        {upcomingSchedules.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 12, color: "var(--text5)", letterSpacing: 2, marginBottom: 10 }}>📅 예정된 러닝</div>
            {upcomingSchedules.map(s => {
              const sc = s.schedule;
              const dday = getDday(sc.date, sc.time);
              const ddayColor = dday === "D-Day" ? "#ff6b6b" : dday === "D-1" ? "#ffaa00" : "#22D3EE";
              return (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "var(--accent-bg)", border: "1px solid var(--accent-bd)", borderRadius: 14, marginBottom: 8 }}>
                  <div style={{ width: 46, height: 46, borderRadius: 12, background: "#0a1820", border: `1.5px solid ${ddayColor}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <div style={{ fontSize: 11, fontWeight: 900, color: ddayColor, letterSpacing: -0.5 }}>{dday}</div>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "var(--text1)", marginBottom: 3 }}>{sc.title}</div>
        ...
        {notifications.map(n => {
          ...
          const typeText = n.type === "reaction" ? ` 님이 ${n.emoji} 반응했어요`
            ...
            : " 님이 메시지를 보냈어요";
          return (
            <div key={n.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: n.read ? "var(--bg)" : "#0a1a0f", border: `1px solid ${n.read ? "#111" : "#155e75"}`, borderRadius: 14, marginBottom: 8 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 15, lineHeight: 1.5 }}>
                  ...
                  <span style={{ color: "var(--text3)" }}>{typeText}</span>
                </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                {!n.read && <div style={{ width: 7, height: 7, borderRadius: 4, background: "var(--accent)" }} />}
                {onDelete && (
                  <button onClick={() => onDelete(n.id)} style={{ background: "none", border: "none", color: "var(--text5)", fontSize: 18, width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 14, cursor: "pointer" }}>✕</button>
                )}
              </div>
            </div>
          );
...
function ChatTab({ setId, currentUser }) {
  ...
  return (
    <div ref={containerRef} style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, background: "var(--bg)", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px 8px", WebkitOverflowScrolling: "touch" }}>
        {messages.length === 0 && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", color: "var(--text4)", gap: 12, padding: "0 32px" }}>
            ...
            }} style={{ background: "var(--accent-bg)", border: "1px solid var(--accent-bd)", borderRadius: 14, padding: "11px 22px", color: "var(--accent)", fontFamily: "inherit", fontSize: 14, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 8, marginTop: 4 }}>
              🔗 크루원 초대하기
            </button>
          </div>
        )}
            ...
            return (
              <div key={msg.id}>
                ...
                <div style={{ margin: "8px 0", padding: "0 4px" }}>
                  <div style={{ background: "var(--modal-bg)", border: "1px solid var(--accent-bd)", borderRadius: 16, overflow: "hidden" }}>
                    {/* 카드 헤더 */}
                    <div style={{ background: "linear-gradient(135deg,#0a1f14,#061209)", padding: "12px 14px 10px", borderBottom: "1px solid #1a2a1a" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        ...
                        {sc.place && <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "var(--text3)" }}>
                    ...
                    <div style={{ padding: "10px 14px" }}>
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
                        {participants.map(p => (
                          <div key={p.uid} style={{ display: "flex", alignItems: "center", gap: 4, background: "var(--bg4)", borderRadius: 20, padding: "3px 8px 3px 4px", border: "1px solid #1e1e1e" }}>
                            <span style={{ fontSize: 14 }}>{p.avatar}</span>
                            <span style={{ fontSize: 12, color: "var(--text3)" }}>{p.name}</span>
                          </div>
                        ))}
                        {participants.length === 0 && <span style={{ fontSize: 12, color: "var(--text5)" }}>아직 참여자가 없어요</span>}
                      </div>
                      {!(sc.closed || isExpired) && (
                        <button
                          ...
                          style={{ width: "100%", padding: "10px", borderRadius: 10, border: "none", fontFamily: "inherit", fontSize: 14, fontWeight: 700,
                            background: isJoined ? "#111" : "#22D3EE",
            ...
            <div key={msg.id}>
              ...
              <div style={{ display: "flex", flexDirection: isMe ? "row-reverse" : "row", alignItems: "flex-end", gap: 6, marginBottom: isLastInGroup ? 8 : 2 }}>
                ...
                <div style={{ maxWidth: "75%", display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start", gap: 1 }}>
                  ...
                  <div style={{
                    background: isMe ? "#22D3EE" : "var(--border)",
      ...
      {/* 입력창 */}
      <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "10px 14px", paddingBottom: `calc(10px + ${safeBottom})`, paddingRight: "14px", borderTop: "1px solid var(--border)", background: "var(--bg)", flexShrink: 0 }}>
        <button onClick={() => setShowScheduleModal(true)}
          style={{ width: 42, height: 42, borderRadius: 21, background: "var(--modal-bg)", border: "1px solid var(--accent-bd)", color: "var(--accent2)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>
          📅
        </button>
        <input
          ref={inputRef}
        ...
        <button onClick={handleSend} disabled={!text.trim() || sending}
          style={{ width: 42, height: 42, borderRadius: 21, background: text.trim() ? "#22D3EE" : "#111", border: "none", color: text.trim() ? "#000" : "#333", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.15s" }}>
...
function ScheduleCreateModal({ onClose, onCreate }) {
  ...
  const labelStyle = { fontSize: 12, color: "var(--text4)", letterSpacing: 1, marginBottom: 6 };
  const inputStyle = { width: "100%", background: "var(--bg2)", border: "1px solid #1e1e1e", borderRadius: 10, padding: "11px 14px", color: "var(--text1)", fontFamily: "inherit", fontSize: 14, outline: "none", boxSizing: "border-box" };

  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "var(--overlay)", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
        ...
        <div style={{ padding: "16px 20px 14px", borderBottom: "1px solid var(--border)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button onClick={handleCreate} disabled={!title.trim() || !date || loading}
              style={{ background: "none", border: "none", color: title.trim() && date ? "#22D3EE" : "#333", fontFamily: "inherit", fontSize: 15, fontWeight: 800 }}>
              {loading ? "저장 중..." : "저장"}
            </button>
          </div>
        </div>
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        {[
          ...
          <div key={l} style={{ background: "var(--bg)", border: "1px solid #161616", borderRadius: 16, padding: "16px 14px" }}>
            <div style={{ fontSize: 13, marginBottom: 6 }}>{icon}</div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "var(--accent)" }}>{v}</div>
            <div style={{ fontSize: 12, color: "var(--text5)", marginTop: 4 }}>{l}</div>
          </div>
        ))}
      </div>
      ...
      {isPro ? (
        <div style={{ background: "var(--bg)", border: "1px solid #161616", borderRadius: 16, padding: "16px", marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ fontSize: 13, color: "var(--text4)", letterSpacing: 1 }}>🎯 이번 주 목표</div>
            <button onClick={() => { setGoalInput(goal || ""); setShowGoalEdit(true); }}
              style={{ fontSize: 12, color: "var(--accent)", background: "none", border: "1px solid var(--accent-bd)", borderRadius: 8, padding: "3px 10px", fontFamily: "inherit" }}>
              {goal > 0 ? "수정" : "설정하기"}
            </button>
          </div>
          {goal > 0 ? (
            <>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: 20, fontWeight: 800, color: "var(--accent)" }}>{thisWeekDist.toFixed(1)}km</span>
                <span style={{ fontSize: 14, color: "var(--text5)" }}>/ {goal}km</span>
              </div>
              <div style={{ background: "var(--bg4)", borderRadius: 8, height: 10, overflow: "hidden" }}>
                <div style={{ width: `${goalPct}%`, height: "100%", background: goalPct >= 100 ? "#22D3EE" : "linear-gradient(90deg,#00aa55,#00ff88)", borderRadius: 8, transition: "width 0.5s" }} />
              </div>
              <div style={{ fontSize: 12, color: goalPct >= 100 ? "#22D3EE" : "#444", marginTop: 6, textAlign: "right" }}>
        ...
        <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 16, padding: "16px", marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {[["week", "주간"], ["month", "월별 " + (!isPro ? "🔒" : "")], ["year", "연별 " + (!isPro ? "🔒" : "")]].map(([v, l]) => {
            ...
            return (
              <button key={v} onClick={() => !locked && setChartView(v)}
                style={{ flex: 1, padding: "8px", borderRadius: 10, border: "none", background: chartView === v ? "#22D3EE" : "#111", color: chartView === v ? "#000" : locked ? "var(--text4)" : "#555", fontFamily: "inherit", fontSize: 13, fontWeight: 700 }}>
                {l}
              </button>
            );
          })}
          ...
          {chartData.map((w, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                {w.dist > 0 && <div style={{ fontSize: 9, color: isLast ? "#22D3EE" : "#444" }}>{w.dist.toFixed(1)}</div>}
                <div style={{ width: "100%", height: h, background: isLast ? "#22D3EE" : "#1a3020", borderRadius: "3px 3px 0 0", minHeight: h > 0 ? 4 : 0 }} />
                <div style={{ fontSize: 9, color: isLast ? "#22D3EE" : "var(--text4)", textAlign: "center", wordBreak: "keep-all" }}>{w.label}</div>
              </div>
            );
          })}
        </div>
      ...
      {/* 🏅 배지/업적 */}
      <div style={{ background: "var(--bg)", border: "1px solid #161616", borderRadius: 16, padding: "16px", marginBottom: 16 }}>
        <div style={{ fontSize: 13, color: "var(--text4)", letterSpacing: 1, marginBottom: 12 }}>🏅 업적 배지</div>
        {(() => {
          ...
          return (
            <>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: locked.length > 0 ? 10 : 0 }}>
                {earned.map(b => (
                  <div key={b.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, background: "#0a1820", border: "1px solid var(--accent-bd)", borderRadius: 12, padding: "10px 12px", minWidth: 72 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {locked.map(b => (
                  <div key={b.id} style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--bg2)", border: "1px solid #161616", borderRadius: 10, padding: "6px 10px", opacity: 0.5 }}>
                    <span style={{ fontSize: 16, filter: "grayscale(1)" }}>{b.emoji}</span>
                    <span style={{ fontSize: 11, color: "var(--text5)" }}>{b.desc}</span>
                  </div>
                ))}
              </div>
            </>
      ...
      {isPro ? (() => {
        ...
        const zones = [
          ...
          { name: "적당",   color: "var(--accent)", range: [5, 6],   desc: "5분~6분" },
              ...
              : counts.map(z => (
                <div key={z.name} style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 13, color: "var(--text3)" }}>{z.name} <span style={{ fontSize: 11, color: "var(--text4)" }}>{z.desc}</span></span>
      ...
      {isPro ? (() => {
        ...
        return (
          <div style={{ background: "var(--bg)", border: "1px solid #161616", borderRadius: 16, padding: "16px", marginBottom: 16 }}>
            <div style={{ fontSize: 13, color: "var(--text4)", letterSpacing: 1, marginBottom: 12 }}>👥 크루 비교</div>
            {metrics.map(m => (
              <div key={m.label} style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
                ...
                <div style={{ flex: 1, display: "flex", gap: 6, alignItems: "center" }}>
                  <div style={{ flex: 1, background: "#0a1820", border: "1px solid var(--accent-bd)", borderRadius: 8, padding: "6px 10px", textAlign: "center" }}>
      ...
      {/* 목표 설정 모달 */}
      {showGoalEdit && (
        <div onClick={() => setShowGoalEdit(false)} style={{ position: "fixed", inset: 0, background: "var(--overlay)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 32px" }}>
          <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 320, background: "var(--bg4)", borderRadius: 20, padding: "24px 20px", border: "1px solid var(--border2)" }}>
            <div style={{ fontSize: 16, fontWeight: 800, marginBottom: 16 }}>🎯 주간 목표 설정</div>
            <div style={{ fontSize: 13, color: "var(--text4)", marginBottom: 8 }}>목표 거리 (km/주)</div>
            <input type="number" value={goalInput} onChange={e => setGoalInput(e.target.value)}
              placeholder="예: 20" min="1" max="300"
              style={{ width: "100%", background: "var(--bg2)", border: "1px solid var(--border2)", borderRadius: 10, padding: "12px 14px", color: "var(--text1)", fontFamily: "inherit", fontSize: 16, outline: "none", boxSizing: "border-box", marginBottom: 16 }} />
            <div style={{ display: "flex", gap: 10 }}>
    ...
    <div style={{ background: "var(--bg)", border: "1px solid #161616", borderRadius: 16, padding: "16px", marginBottom: 16 }}>
      ...
      {monthPosts.length === 0 ? (
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0" }}>
          ...
          <div style={{ fontSize: 14, color: "var(--text4)" }}>🤖 AI가 분석 중이에요...</div>
        </div>
      ) : !generated ? (
        ...
        <div>
          {(report || "").split("\n").filter(l => l.trim()).map((line, i) => (
            <div key={i} style={{ fontSize: 14, color: "var(--text2)", lineHeight: 1.8, marginBottom: 4 }}>{line}</div>
          ))}
          <button onClick={() => { setReport(null); setGenerated(false); }}
            style={{ marginTop: 10, fontSize: 12, color: "var(--text4)", background: "none", border: "1px solid var(--border2)", borderRadius: 8, padding: "4px 12px", fontFamily: "inherit" }}>
            다시 생성
          </button>
        </div>
      )}
...
function ProfileModal({ currentUser, posts, currentSet, isAdmin, onKick, onTransfer, onLeaveSet, onDeleteSet, onClose, onLogout, onUpdateProfile, onRedeemCoupon }) {
  ...
  const handlePhotoChange = async (e) => {
    ...
    // ✅ Optimistic Update: 즉시 로컬 미리보기 표시
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "var(--modal-bg)", borderRadius: "22px 22px 0 0", border: "1px solid var(--border)", padding: "16px 18px", paddingBottom: `calc(20px + ${safeBottom})`, maxHeight: "70vh", overflowY: "auto", position: "relative" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div style={{ display: "flex", gap: 14, alignItems: "center", flex: 1 }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              <div style={{ width: 70, height: 70, borderRadius: 35, background: "var(--bg4)", border: "1.5px solid #222", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 34, overflow: "hidden" }}>
                {photoURL
              ...
              {editMode && (
                <>
                  ...
                  <button onClick={() => photoInputRef.current?.click()} disabled={photoUploading}
                    style={{ position: "absolute", bottom: 0, right: 0, width: 24, height: 24, borderRadius: 12, background: photoUploading ? "#333" : "#22D3EE", border: "2px solid #0d0d0d", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#000", padding: 0 }}>
                    {photoUploading ? "⏳" : "📷"}
                  </button>
                </>
              )}
            ...
            <div style={{ flex: 1 }}>
              {editMode ? <input value={name} onChange={e => setName(e.target.value)} style={{ background: "transparent", border: "none", borderBottom: "1px solid #333", color: "var(--text1)", fontFamily: "inherit", fontSize: 19, fontWeight: 800, outline: "none", width: "100%" }} />
                : <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    {currentUser?.isPro && <span style={{ background: "transparent", border: "1px solid var(--accent)", color: "var(--accent)", borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 800, letterSpacing: 1 }}>PRO</span>}
                  </div>}
              {editMode ? <input value={bio} onChange={e => setBio(e.target.value)} placeholder="한 줄 소개..." style={{ background: "transparent", border: "none", borderBottom: "1px solid #222", color: "var(--text3)", fontFamily: "inherit", fontSize: 14, outline: "none", width: "100%", marginTop: 6 }} />
                : <div style={{ fontSize: 14, color: "var(--text3)", marginTop: 4 }}>{currentUser?.bio || "한 줄 소개를 입력해보세요"}</div>}
            </div>
          </div>
          <button onClick={() => editMode ? saveProfile() : setEditMode(true)} style={{ background: editMode ? "#22D3EE" : "#111", border: "1px solid #1e1e1e", borderRadius: 10, padding: "9px 16px", color: editMode ? "#000" : "#666", fontFamily: "inherit", fontSize: 15, fontWeight: 700, minHeight: 42 }}>
            {editMode ? "저장" : "편집"}
          </button>
        </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                  ...
                  <span style={{ background: "transparent", border: "1px solid var(--accent)", color: "var(--accent)", borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 800, letterSpacing: 1 }}>PRO</span>
                </div>
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                {[
                  ...
                  <button key={p.type} onClick={() => setPayPlan(p.type)}
                    style={{ flex: 1, padding: "12px 8px", borderRadius: 14, border: payPlan === p.type ? "1.5px solid #00ff88" : "1px solid #222", background: payPlan === p.type ? "#0a1820" : "var(--bg2)", textAlign: "center", position: "relative" }}>
                    ...
                    <div style={{ fontSize: 13, color: payPlan === p.type ? "#22D3EE" : "#555", fontWeight: 700 }}>{p.label}</div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={handleRedeem} disabled={!couponCode.trim() || couponLoading}
                  style={{ padding: "10px 16px", borderRadius: 10, background: couponCode.trim() ? "#22D3EE" : "#111", border: "none", color: couponCode.trim() ? "#000" : "#333", fontFamily: "inherit", fontSize: 14, fontWeight: 800, flexShrink: 0 }}>
                  {couponLoading ? "..." : "적용"}
                </button>
              </div>
              {couponMsg && <div style={{ marginTop: 8, fontSize: 13, color: couponMsg.ok ? "#22D3EE" : "#ff4444", fontWeight: 600 }}>{couponMsg.text}</div>}
            </>
          )}
        </div>
        ...
        {currentSet && (
          <div style={{ marginTop: 18 }}>
            <div style={{ fontSize: 12, color: "var(--text5)", letterSpacing: 2, marginBottom: 12 }}>
              {isAdmin ? "👑 멤버 관리" : "👥 멤버 목록"}
            </div>
            {(currentSet.members || []).map(m => (
              <div key={m.uid} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "var(--bg)", border: "1px solid #111", borderRadius: 12, marginBottom: 6 }}>
                <Avatar user={m} size={34} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 700 }}>
                    {m.name}
                    {m.uid === currentUser?.uid && <span style={{ color: "var(--accent)", fontSize: 12, marginLeft: 6 }}>나</span>}
            ...
            {!isAdmin && (
              <button onClick={onLeaveSet} style={{ width: "100%", marginTop: 8, padding: "13px", background: "transparent", border: "1px solid var(--border3)", borderRadius: 14, color: "var(--text3)", fontFamily: "inherit", fontSize: 15, minHeight: 48 }}>크루 나가기</button>
            )}
          </div>
        )}
    ...
    <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "var(--bg)", borderTop: "1px solid var(--border)", paddingBottom: safeBottom, display: "flex", alignItems: "center", zIndex: 100, backdropFilter: "blur(20px)" }}>
      {tabs.slice(0, 2).map(t => (
        <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "10px 0 8px", border: "none", background: "transparent", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, minHeight: 58 }}>
          ...
          <span style={{ fontSize: 11, color: tab === t.id ? "#22D3EE" : "#888", fontWeight: tab === t.id ? 700 : 500 }}>{t.label}</span>
        </button>
      ))}

      {/* 중앙 + 버튼 */}
      <div style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "center", paddingBottom: 4 }}>
        <button onClick={onUpload} style={{ width: 52, height: 52, borderRadius: 26, background: "var(--accent)", border: "none", color: "#000", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 20px rgba(0,255,136,0.4)", marginTop: -10 }}>
...
export default function CommunityApp({ currentUser, currentSet, onLeaveSet, onLogout, onUpdateProfile, onRedeemCoupon }) {
  ...
  return (
    <div style={{ height: "100dvh", background: "var(--bg)", color: "var(--text1)", fontFamily: "'Pretendard', -apple-system, sans-serif", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", overflow: "hidden", position: "fixed", inset: "0 auto", width: "100%" }}>
      ...
      <div style={{ padding: `calc(14px + ${safeTop}) 18px 10px`, display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, background: "var(--bg)", zIndex: 50 }}>
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 800, color: "var(--accent)", letterSpacing: -0.5 }}>{currentSet?.emoji} {currentSet?.name}</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {/* 알림 벨 */}
          <div style={{ position: "relative", cursor: "pointer" }} onClick={() => setShowNotif(true)}>
            <div style={{ width: 36, height: 36, borderRadius: 18, background: "var(--modal-bg)", border: "1px solid #1e1e1e", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {/* 주간 요약 - 채팅 탭에서는 숨김 */}
      <div style={{ margin: "0 18px 12px", background: "var(--bg2)", border: "1px solid #161616", borderRadius: 16, padding: "12px 16px", display: tab === "chat" ? "none" : "flex", flexShrink: 0 }}>
        {[[`${myWeekDist.toFixed(1)}km`, "이번 주"], [`${posts.filter(p => { if (p.userId !== currentUser?.uid) return false; const ts = p.createdAt?.toDate ? p.createdAt.toDate() : new Date(p.createdAt||0); return ts.getTime() >= getThisWeekStart(); }).length}회`, "이번 주 횟수"], [`${calcStreak(posts, currentUser?.uid)}일`, "🔥 스트릭"]].map(([v, l], i) => (
          <div key={l} style={{ flex: 1, borderLeft: i > 0 ? "1px solid #141414" : "none", paddingLeft: i > 0 ? 14 : 0 }}>
            <div style={{ fontSize: 19, fontWeight: 800, color: "var(--accent)" }}>{v}</div>
      ...
      <div style={{ flex: 1, overflowY: tab === "chat" ? "hidden" : "auto", WebkitOverflowScrolling: "touch", display: "flex", flexDirection: "column", padding: tab === "chat" ? "0" : "0 18px", paddingBottom: tab === "chat" ? "0" : `calc(80px + ${safeBottom})`, minHeight: 0 }}>
        {loading && (
          <div style={{ display: "flex", justifyContent: "center", paddingTop: 60 }}>
            <div style={{ width: 34, height: 34, border: "2px solid var(--accent)", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
            ...
            {posts.length === 0 && (
              <div style={{ textAlign: "center", padding: "50px 0 40px", color: "var(--text4)" }}>
                ...
                <div style={{ fontSize: 14, color: "var(--text4)", lineHeight: 1.9, marginBottom: 28 }}>
                  아래 <span style={{ color: "var(--accent)", fontWeight: 800 }}>+</span> 버튼으로 러닝 기록을 올리거나<br />
                  크루원을 초대해서 같이 달려보세요!
                </div>
                <button onClick={() => {
                  const link = getInviteLink(currentSet?.id);
                ...
                }} style={{ background: "var(--accent-bg)", border: "1px solid var(--accent-bd)", borderRadius: 14, padding: "13px 24px", color: "var(--accent)", fontFamily: "inherit", fontSize: 15, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 8 }}>
                  🔗 크루원 초대하기
                </button>
              </div>
            )}
